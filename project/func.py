...
def segments2energy_consumption(segments, energy_prices_df, kind='remaining'):
    ...
    budget_share_seg = ds_mul_df(area * energy_consumption_conventional, energy_prices_df / income_ts)
...
def energy_consumption2cost(energy_consumption, energy_prices_df):
    """Returns energy cost segmented and for every year based on energy consumption and energy prices.
    """
    energy_prices = reindex_mi(energy_prices_df.T, energy_consumption.index, ['Heating energy'])
...
def segments2energy_lcc(segments, energy_prices_df, yr, kind='remaining', transition='label'):
    ...
    energy_consumption_seg = segments2energy_consumption(segments, energy_prices_df, kind=kind)['Consumption-actual']
    energy_cost_ts_df = energy_consumption2cost(energy_consumption_seg, energy_prices_df)
...
def segments2renovation_rate(segments, yr, energy_prices_df, cost_invest_df, cost_intangible_df, rho):
...
def segments_new2lcc(segments_new, yr, energy_prices_df, cost_new=cost_dict['cost_new']):
    ...
    cost_new_reindex = reindex_mi(cost_new, energy_lcc_new_ds.index,
...
def renovation_label2renovation_label_energy(energy_lcc_ds, cost_switch_fuel_df, stock_renovation_label):
...
def housing_need2housing_construction(nb_housing_need, nb_housing_construction, share_multi_family, yr, yr_ini):
    ...
    housing_share_tot_new = pd.Series([share_multi_family_construction, 1 - share_multi_family_construction],
                                      index=['Multi-family', 'Single-family'])
    # share of occupancy status in housing type is constant
    dm_share_tot_construction = ds_mul_df(housing_share_tot_new, parameters_dict['os_share_ht_construction']).stack()
...
def segments_new2flow_constructed(flow_constructed_seg_dm, segments_new, energy_prices_df, ds_income_owner_prop,
    ...
    """
    lcc_new_ds = segments_new2lcc(segments_new, yr, energy_prices_df, cost_new=cost_dict['cost_new'])
...
def calibration_market_share(lcc_df, logging):
    ...
    logging.debug('Intangible cost (€/m2): \n {:.0f}'.format(intangible_cost_mean))
    return intangible_cost


def calibration_renovation_rate(npv_df):
...
def seg_share_construction_func():
    ...
    return de_aggregating_series(os_he_ht_share_tot_construction, parameters_dict['ep_share_tot_construction'],
